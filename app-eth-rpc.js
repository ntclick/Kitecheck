...
class SimpleRankChecker {
    ...
    async getNFTDataETH(address) {
        try {
            ...
            if (response.ok) {
                const data = await response.json();
                console.log('✅ NFT data fetched:', data.result?.length || 0, 'transfers');
                return data;
            } else {
                throw new Error(`HTTP ${response.status}`);
            }
    ...
    calculateScoreBreakdown(accountData) {
        ...
        const totalPoints = Math.floor((balancePoints + transactionPoints + activityPoints + nftPoints + tokenPoints) * nftMultiplier);
